...
export const useThemeToggle = ({
  ...
  const toggleTheme = useCallback((coords?: AnimationCoords) => {
    setIsDark(!isDark);

    const animation = createAnimation(variant, coords || start, blur, gifUrl);
  ...
  const setCrazyLightTheme = useCallback((overRideOptions?: { variant?: AnimationVariant; start?: AnimationStart | AnimationCoords; blur?: boolean; gifUrl?: string }) => {
    ...
    const { variant = 'circle', start = 'center', blur = false, gifUrl = '' } = overRideOptions || {};
...
import { AnimationVariant, AnimationStart, AnimationCoords } from '@/types/theme';
...
// Helper to check if start is coordinate-based
const isAnimationCoords = (start: AnimationStart | AnimationCoords): start is AnimationCoords => {
  return typeof start === 'object' && 'x' in start && 'y' in start;
};

export const createAnimation = (
  variant: AnimationVariant,
  start: AnimationStart | AnimationCoords = 'center',
  blur = false,
  url?: string,
): Animation => {
  // Handle dynamic pixel coordinates - always use circle animation
  if (isAnimationCoords(start)) {
    const { x, y } = start;
    const animName = `coords-${x}-${y}${blur ? '-blur' : ''}`;
    return {
      name: animName,
      css: `
      ...
      ::view-transition-new(root) {
        animation-name: reveal-light-coords${blur ? '-blur' : ''};
      ...
      .dark::view-transition-new(root) {
        animation-name: reveal-dark-coords${blur ? '-blur' : ''};
        ${blur ? 'filter: blur(2px);' : ''}
      }

      @keyframes reveal-dark-coords${blur ? '-blur' : ''} {
        from {
      ...
      @keyframes reveal-light-coords${blur ? '-blur' : ''} {
        from {
           clip-path: circle(0% at ${x}px ${y}px);
           ${blur ? 'filter: blur(8px);' : ''}
        }
